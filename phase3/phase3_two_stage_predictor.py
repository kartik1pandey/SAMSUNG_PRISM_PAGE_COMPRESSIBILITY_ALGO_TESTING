"""
Phase 3: Two-Stage Predictor
Stage-1: Fast feature-based filtering (rejects obvious incompressible)
Stage-2: Sample compression for ambiguous cases
"""
import os
import sys
import numpy as np
import pandas as pd
import lz4.frame
from collections import Counter
from tqdm import tqdm

# Add parent directory to path to import from phase2
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))

PAGE_SIZE = 4096
SAMPLE_SIZE_STAGE1 = 64
SAMPLE_SIZE_STAGE2 = 256
BETA_STAGE2 = 0.75  # Compression ratio threshold for Stage-2

# Stage-1 thresholds (optimized from Phase 2)
STAGE1_THRESHOLDS = {
    "distinct_max": 80,
    "max_freq_ratio": 0.05,
    "zero_ratio": 0.20,
    "run_count": 5
}

# Scoring weights for Stage-1
FEATURE_WEIGHTS = {
    "distinct_score": -1.0,
    "max_freq_score": 2.0,
    "zero_score": 1.5,
    "run_score": 1.0
}

STAGE1_CONFIDENCE_THRESHOLD_HIGH = 2.5  # High score → definitely compressible
STAGE1_CONFIDENCE_THRESHOLD_LOW = 0.5   # Low score → definitely incompressible
# Between thresholds → ambiguous, send to Stage-2

# ============== Stage-1: Fast Feature-Based Filtering ==============

def sample_page_stage1(page_bytes):
    """Hybrid sampling: first 64 + random 64 bytes"""
    sample1 = page_bytes[:SAMPLE_SIZE_STAGE1]
    offset = np.random.randint(0, PAGE_SIZE - SAMPLE_SIZE_STAGE1)
    sample2 = page_bytes[offset:offset + SAMPLE_SIZE_STAGE1]
    return sample1 + sample2

def extract_features(sample):
    """Extract integer-only features"""
    counts = Counter(sample)
    distinct = len(counts)
    max_freq_ratio = max(counts.values()) / len(sample)
    zero_ratio = counts.get(0, 0) / len(sample)
    run_count = sum(1 for i in range(1, len(sample)) if sample[i] == sample[i-1])
    
    return {
        "distinct": distinct,
        "max_freq_ratio": max_freq_ratio,
        "zero_ratio": zero_ratio,
        "run_count": run_count
    }

def compute_compressibility_score(feats, weights=FEATURE_WEIGHTS):
    """Compute weighted compressibility score"""
    distinct_norm = 1.0 - (feats["distinct"] / 128.0)
    max_freq_norm = feats["max_freq_ratio"]
    zero_norm = feats["zero_ratio"]
    run_norm = min(feats["run_count"] / 50.0, 1.0)
    
    score = (
        weights["distinct_score"] * distinct_norm +
        weights["max_freq_score"] * max_freq_norm +
        weights["zero_score"] * zero_norm +
        weights["run_score"] * run_norm
    )
    
    return score

def stage1_predict(page_bytes):
    """
    Stage-1 prediction with confidence levels
    Returns: (decision, score)
    - decision: "compressible", "incompressible", or "ambiguous"
    - score: compressibility score
    """
    sample = sample_page_stage1(page_bytes)
    feats = extract_features(sample)
    score = compute_compressibility_score(feats)
    
    if score > STAGE1_CONFIDENCE_THRESHOLD_HIGH:
        return "compressible", score, feats
    elif score < STAGE1_CONFIDENCE_THRESHOLD_LOW:
        return "incompressible", score, feats
    else:
        return "ambiguous", score, feats

# ============== Stage-2: Sample Compression ==============

def stage2_predict(page_bytes, num_samples=2):
    """
    Stage-2: Compress random samples to estimate compressibility
    Takes multiple random samples and compresses them
    Returns: (decision, avg_ratio)
    """
    ratios = []
    
    for _ in range(num_samples):
        offset = np.random.randint(0, PAGE_SIZE - SAMPLE_SIZE_STAGE2)
        sample = page_bytes[offset:offset + SAMPLE_SIZE_STAGE2]
        
        compressed = lz4.frame.compress(sample, compression_level=16)
        ratio = len(compressed) / SAMPLE_SIZE_STAGE2
        ratios.append(ratio)
    
    avg_ratio = np.mean(ratios)
    decision = "compressible" if avg_ratio <= BETA_STAGE2 else "incompressible"
    
    return decision, avg_ratio

# ============== Two-Stage Predictor ==============

def two_stage_predict(page_bytes):
    """
    Full two-stage prediction pipeline
    Returns: (final_decision, stage_used, stage1_score, stage2_ratio)
    """
    # Stage-1
    stage1_decision, stage1_score, stage1_feats = stage1_predict(page_bytes)
    
    if stage1_decision == "compressible":
        return "compressible", "stage1", stage1_score, None, stage1_feats
    elif stage1_decision == "incompressible":
        return "incompressible", "stage1", stage1_score, None, stage1_feats
    else:  # ambiguous
        # Stage-2
        stage2_decision, stage2_ratio = stage2_predict(page_bytes)
        return stage2_decision, "stage2", stage1_score, stage2_ratio, stage1_feats

# ============== Evaluation ==============

def evaluate_two_stage(df, pages_dir):
    """Run two-stage predictor on entire dataset"""
    
    print("Running Two-Stage Predictor...")
    print(f"Stage-1 confidence thresholds: [{STAGE1_CONFIDENCE_THRESHOLD_LOW}, {STAGE1_CONFIDENCE_THRESHOLD_HIGH}]")
    print(f"Stage-2 compression threshold: β = {BETA_STAGE2}")
    print()
    
    predictions = []
    stages_used = []
    stage1_scores = []
    stage2_ratios = []
    features_list = []
    
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Predicting"):
        page_path = os.path.join(pages_dir, row["page_id"])
        
        with open(page_path, "rb") as f:
            page_bytes = f.read()
        
        pred, stage, s1_score, s2_ratio, feats = two_stage_predict(page_bytes)
        
        predictions.append(pred)
        stages_used.append(stage)
        stage1_scores.append(s1_score)
        stage2_ratios.append(s2_ratio if s2_ratio is not None else -1)
        features_list.append(feats)
    
    df["two_stage_pred"] = predictions
    df["stage_used"] = stages_used
    df["stage1_score"] = stage1_scores
    df["stage2_ratio"] = stage2_ratios
    
    # Add Stage-1 features
    for feat_name in ["distinct", "max_freq_ratio", "zero_ratio", "run_count"]:
        df[f"feat_{feat_name}"] = [f[feat_name] for f in features_list]
    
    return df

def compute_metrics(df):
    """Compute prediction metrics"""
    accuracy = (df["two_stage_pred"] == df["label"]).mean()
    
    tp = ((df["label"] == "compressible") & (df["two_stage_pred"] == "compressible")).sum()
    tn = ((df["label"] == "incompressible") & (df["two_stage_pred"] == "incompressible")).sum()
    fp = ((df["label"] == "incompressible") & (df["two_stage_pred"] == "compressible")).sum()
    fn = ((df["label"] == "compressible") & (df["two_stage_pred"] == "incompressible")).sum()
    
    precision = tp / (tp + fp) if (tp + fp) > 0 else 0
    recall = tp / (tp + fn) if (tp + fn) > 0 else 0
    fpr = fp / (fp + tn) if (fp + tn) > 0 else 0
    
    return {
        "accuracy": accuracy,
        "precision": precision,
        "recall": recall,
        "fpr": fpr,
        "tp": tp,
        "tn": tn,
        "fp": fp,
        "fn": fn
    }

def main():
    print("="*60)
    print("PHASE 3: TWO-STAGE PREDICTOR EVALUATION")
    print("="*60)
    print()
    
    # Load dataset
    df = pd.read_csv("page_labels.csv")
    pages_dir = "pages"
    
    print(f"Loaded {len(df)} labeled pages")
    print()
    
    # Run two-stage predictor
    df = evaluate_two_stage(df, pages_dir)
    
    # Compute metrics
    metrics = compute_metrics(df)
    
    print("\n" + "="*60)
    print("TWO-STAGE PREDICTOR RESULTS")
    print("="*60)
    print(f"Accuracy:  {metrics['accuracy']:.4f}")
    print(f"Precision: {metrics['precision']:.4f}")
    print(f"Recall:    {metrics['recall']:.4f}")
    print(f"FPR:       {metrics['fpr']:.4f}")
    print()
    
    print("Confusion Matrix:")
    print(f"  TP: {metrics['tp']:5d}  |  FP: {metrics['fp']:5d}")
    print(f"  FN: {metrics['fn']:5d}  |  TN: {metrics['tn']:5d}")
    print()
    
    # Stage usage statistics
    print("="*60)
    print("STAGE USAGE STATISTICS")
    print("="*60)
    stage_counts = df['stage_used'].value_counts()
    total = len(df)
    for stage, count in stage_counts.items():
        print(f"  {stage:10s}: {count:5d} / {total} ({count/total*100:.2f}%)")
    print()
    
    # Per-type accuracy
    print("="*60)
    print("ACCURACY BY PAGE TYPE")
    print("="*60)
    for ptype in sorted(df['page_type'].unique()):
        subset = df[df['page_type'] == ptype]
        acc = (subset["two_stage_pred"] == subset["label"]).mean()
        stage2_pct = (subset['stage_used'] == 'stage2').sum() / len(subset) * 100
        print(f"  {ptype:10s}: {acc:.4f} (Stage-2: {stage2_pct:.1f}%)")
    print()
    
    # Comparison with Stage-1 only
    print("="*60)
    print("IMPROVEMENT OVER STAGE-1 ONLY")
    print("="*60)
    
    # Simulate Stage-1 only (using high threshold)
    df['stage1_only_pred'] = df['stage1_score'].apply(
        lambda x: "compressible" if x > 0.0 else "incompressible"
    )
    stage1_only_acc = (df['stage1_only_pred'] == df['label']).mean()
    
    print(f"Stage-1 only accuracy: {stage1_only_acc:.4f}")
    print(f"Two-stage accuracy:    {metrics['accuracy']:.4f}")
    print(f"Improvement:           {(metrics['accuracy'] - stage1_only_acc)*100:.2f}%")
    print()
    
    # Save results
    df.to_csv("phase3/phase3_two_stage_results.csv", index=False)
    print("✅ Saved detailed results to 'phase3/phase3_two_stage_results.csv'")

if __name__ == "__main__":
    main()
